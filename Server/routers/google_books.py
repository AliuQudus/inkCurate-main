...
#     """Helper to contact Google Books API with timeout and basic error handling."""
#     if GOOGLE_API_KEY:
#         params["key"] = GOOGLE_API_KEY
#     try:
#         r = requests.get(GOOGLE_BOOKS_BASE, params=params, timeout=5)
#     except requests.RequestException as e:
#         raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Google Books request failed: {e}")
#     if r.status_code != 200:
#         raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Google Books returned {r.status_code}")
#     return r.json()

# def extract_book_summary(item: dict) -> dict:
#     """Return small JSON for frontend from a Google Books 'item'."""
...
# def search_books(q: str = Query(..., min_length=1), max_results: int = Query(10, le=40)):
#     """
#     Search books via Google Books API.
#     Example: GET /google/search?q=harry+potter&max_results=10
#     Returns list of small book objects (google_book_id, title, authors, thumbnail, ...)
#     """
#     params = {"q": q, "maxResults": max_results}
#     raw = google_books_request(params)
#     items = raw.get("items", [])
#     return [extract_book_summary(it) for it in items]

# @router.get("/books/{google_book_id}", response_model=dict)
# def get_book_detail(google_book_id: str = Path(...)):
#     """
#     Fetch full details for a single Google Book ID.
#     You can use this to show a details page before liking.
#     """
#     raw = google_books_request({"q": f"id:{google_book_id}"})
#     items = raw.get("items") or []
#     if not items:
#         raise HTTPException(status_code=404, detail="Book not found in Google Books")
#     return extract_book_summary(items[0])
...
def extract_book_summary(item: dict, db: Session, current_user_id: Optional[int] = None) -> dict:
    ...
    # Join authors as a comma-separated string for compatibility with your Book model
    authors_list = info.get("authors") or []
    
    # Check if book exists in DB and get vote info
...
def search_books(
    ...
    # For now, don't require authentication
